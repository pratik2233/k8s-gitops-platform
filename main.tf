...
resource "aws_key_pair" "eks_worker_key" {
  ...
  tags = merge(local.common_tags, {
    Name = "${var.project_name}-worker-key"
  })
}
...
resource "aws_security_group" "eks_worker_remote_access" {
  name        = "${var.project_name}-worker-ssh-sg"
  description = "Allow SSH access to EKS worker nodes for Ansible"
  vpc_id      = aws_vpc.main.id

  ingress {
    description = "SSH from local machine public IP"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
...
resource "aws_eks_node_group" "main" {
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.eks_worker_remote_access.id]
  }
  ...
  tags = merge(local.common_tags, {
    ...
    "AnsibleManaged"                    = "true"
    "ansible_group"                     = "eks_worker_nodes"
